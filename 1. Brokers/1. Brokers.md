...
Whether the outside world in this instance is simply local storage in memory or an independent system that resides behind an API, they all have to reside behind the Brokers in any application.
...
The primitive input parameters will enforce no strong dependencies between the broker-neighboring services and the external models.
...
The general rule here then would be that services can only call brokers, and they can only call external native dependencies.
...
Doing that on the broker level will require pushing business logic into that realm, which is outside that component's purpose.
