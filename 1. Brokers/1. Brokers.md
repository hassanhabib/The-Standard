...
Brokers, in general, are meant to be disposable and replaceable - they are built with the understanding that technology evolves and changes all the time. Therefore,  at some point in the lifecycle of a given application, it will be replaced with a more recent technology that gets the job done faster.
...
The general rule here then would be that services can only call brokers, and the brokers can only call external native dependencies.
...
The primary purpose of this particular organization, leveraging partial classes, is to separate the concern for each entity to an even more refined level, which should make the maintainability of the software much higher.
...
A good example of support brokers is the `DateTimeBroker` - a broker explicitly made to remove the business layer's strong dependency on the system date time.
...
This responsibility lies in the hands of the broker-neighboring services. I also call them foundation services, these services are the last point of abstraction before your core logic that consists of nothing more than local models and contracts.
