...
### 2.1.3.0 Abstraction
The first and most important responsibility for foundation/broker-neighboring services is to ensure a level of abstraction exists between the brkokers and the rest of your system. This abstraction is necessary to ensure the pure business logic layer in any system is verbally and functionaly agnostic to whichever dependencies the system is relying on to communicate with the outside world.

Let's visualize a concerete example of the above principle. Let's assume we have a `StudentProcessingService` which implements an `UpsertStudentAsync` functionality. Somewhere in that implementation there will be a dependency on `AddStudentAsync` which is exposed and implemented by some `StudentService` as a foundation service. Take a look at this snippet:
...
In the above test, we defined four variables with the same value. Each variable contains a name that best fits the context it will be used in. For instnace, `inputStudent` best fits in the input parameter position, while `storageStudent` best first what gets returned from the storage broker after a student is persisted sucessfully.

You will also notice that we deep cloned the `expectedStudent` variable to ensure no modifications have happened to the originally passed in student. For instnace, assume an input student value has changed for any of it's attributes internally within the `AddStudentAsync` function. That change won't trigger a failing test unless we dereference the `expectedStudent` variable from the input and returned variables.
...
```
In the function above, we decided to throw the exception immediately instead of going in further. That's an example of circut-breaking validation type.

But with validations, circut-breaking isn't always the wise thing to do. Sometimes we want to collect all the issues within a particular request before sending the error report back to the request submitter. Let's talk about that in this next section.

#### 2.1.3.1.1 Continuous Validations
Continuous validations are the opposite of circut-breaking validations. They don't stop the flow of validations but they definitely stop the flow of logic. In other words, continuous validations ensure no business logic will be executed but they also ensure other validations of the same type can continue to execute before breaking the circut. Let's materialize this theory with an example:
...
Now that have the upsertable exceptions part is done. This means that we have what it takes to collect our validation errors. But that's not good enough if we don't have a mechanism to break the circut when we believe that the time is right to do so.
We can simply use the native offerings to implement the circut-breaking directly as follows:
...
A non-circut-breaking or continuous validation process will require the ability to pass in dynamic rules at any count or capacity to add these validation errors. A validation rule is a dynamic structure that reports whether the rule has been violated for its condition and the error message that should be reported to the end user to help them fix that issue.

In a scenario where we want to ensure any given Id is valid. A dynamic continueous validation rule would look something like this:
...
Now our Rule doesn't just report whether a particular attribute is invalid or not. But it also has a meaninful human-readable message that helps the consumer of the service understand what that very attribute is invalid.
...
Now that have the advanced exceptions and the dynamic validation rules. It's time to put it all together in terms of accepting infinite number of validation rules, examining their condition results and finally break the circut when all the continueous validations are done. here's how to do that:
...
public class NotFoundStudentException : Xeption
{
	public NotFoundStudentException(Guid id)
		: base ($"Couldn't find a student with id: {id}.")
	{}
}
```
...
In order to implement an external validation we will need to touch on all different aspects of our service. the core logic, the validation and the exception handling aspects as follow.
...
There are situations where attempting to retrieve an entity then finding out that it doesn't exist is not necessarily errorneous. This is where Processing Services come in to leverage a higher-order business logic to deal with this more complex scenario.
...
##### 2.1.3.0.4.1 Implementing Dependency Validations
