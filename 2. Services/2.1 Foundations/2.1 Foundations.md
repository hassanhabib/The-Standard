...
For instance, we shouldn't try to post some `Student` object to an external API if the object is `null`. Or if the `Student` model is corrupted or invalid logically in any way, shape or form.

#### 2.1.3.1.0 Structural Validations
...
The structural validations ensure the data is in a good shape before moving forward with any further validations - for instance, we can't possibly validate a student has the minimum number of characters in their names if their first name is structurally invalid.
...
##### 2.1.3.1.0.0 Testing Structural Validations
...
##### 2.1.3.1.0.1 Implementing Structural Validations
...
##### 2.1.3.1.1.0 Testing Logical Validations
...
public async Task ShouldThrowValidationExceptionOnRegisterIfUpdatedByNotSameAsCreatedByAndLogItAsync()
{
 ...
 ##### 2.1.3.1.1.1 Implementing Logical Validations
...
#### 2.1.3.1.2 Dependency Validations
...
##### 2.1.3.1.2.0 Testing Dependency Validations
