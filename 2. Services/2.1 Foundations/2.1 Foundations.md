...
Here's a native implementation of upserting values in some give dictionary:
...
A non-circut-breaking or continuous validation process will require the ability to pass in dynamic rules at any count or capacity to add these validation errors. A validation rule is a dynamic structure that reports wether the rule has been violated for its condition and the error message that should be reported to the end user to help them fix that issue.
...
public async Task ShouldThrowValidationExceptionOnRegisterIfUpdatedByNotSameAsCreatedByAndLogItAsync()
{
	...
	invalidStudentException.AddData(
		...
		value: $"Id is not the same as {nameof(Student.UpdatedBy)}.");
...
#### 2.1.3.1.4 Dependency Validations
...
##### 2.1.3.1.4.0 Testing Dependency Validations
