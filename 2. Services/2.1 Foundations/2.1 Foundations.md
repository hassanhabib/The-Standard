...
Now that have the upsertable exceptions part is done. This means that we have what it takes to collect our validation errors. But that's not good enough if we don't have a mechanism to break the circuit when we believe that the time is right to do so.
...
Now our Rule doesn't just report whether a particular attribute is invalid or not. But it also has a meaningful human-readable message that helps the consumer of the service understand what that very attribute is invalid.
...
Dependency validations can occur because you called an external resource and it returned an error, or returned a value that warrants raising an error. For instance, an API call might return a `404` code, and that's interpreted as an exception if the input was supposed to correspond to an existing object.
