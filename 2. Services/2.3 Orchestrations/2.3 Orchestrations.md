...
We spoke ealier about orchestration services combining multiple routines to achieve a common purpose or a single flow. This aspect of orchestration services can serve as both a fundamental charactristic but also a responsibility. And orchestration service without at least one routine that combines two or three entities is not considered truly an orchestration. Integrating with multiple services without a common purpose is a better fit definition for Aggregation services which we will discuss later in this services chapter.
...
Just like the previsous services, Orchestration services are responsible during their flow combination to ensure the purity of the exposed input and output contracts. Which becomes a bit more complex when combining multiple models. Orchestration services will continue to be responsible for mapping incoming contracts to their respective downstream serviecs, but also ensures to map back the returned results from these services into the unified model.
...
Note that it's also recommended to break the mapping logic into it's own aspect/partial class file. Something like `StudentOrchestrationService.Mappings.cs` to make sure the only thing left there is the business logic of orchestration.
...
public async ValueTask<LibraryCard> CreateLibraryCardAsync(LibraryCard libraryCard)
{
    ...
    return await this.libraryProcessingCard.CreateLibraryCardAsync(libraryCard);
}
```

ensuring a verify enslisted student exists has happened before creating a library card might become a challenge to achieve naturally because there's no dependency between a return value of one routine and the input parameters of the next. In other words, there is nothing that the `VerifyEnlistedStudentExistAsync` function returns that the `CreateLibraryCardAsync` function cares about in terms of input parameters.
...
public async Task ShouldCreateLibraryCardAsync()
{
    ...
    this.libraryCardProcessingServiceMock.Verify(service =>
        service.VerifyEnlistedStudentExistAsync(inputLibraryCard),
            Times.Once);
...
From the example above, the mock framework here is being used to ensure a certain order is being enforced when it comes to calling these dependencies. This way we enforce a certain implementation within any given method to ensure a non-naturally connected dependencies are being sequencially called in the exact right order.

It's more likely that the form of ordering leans more towards enforced than natural when orchestration services reach the maximum amount of dependencies they may have at any point in time.
