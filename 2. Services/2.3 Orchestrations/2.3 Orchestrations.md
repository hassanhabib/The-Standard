...
### 2.3.2.2 Cul-De-Sac
There are times where Orchestration services and it's equivelant (coordination, management ... etc) may not need an exposer component (controller for instance). That's because these sevices may be listeners to certain events and communicating the event back into a processing or a foundation service at the same level where the event started or was received.

For instance, imagine building a simple application where it gets notified with messages from a queue then maps these messages into some local model to persist it in storage. In this case here, the incoming or input for this services isn't necessarily through an exposer component anymore. The incoming messages can be received from a subscribtion to an event service or a queue. In this case the orchestration service would look something like the following:
...
public async Task ShouldAddProfileAsync()
{
    ...
    {
        Body = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(randomStudent))
    };
 ...
 The test here indicates an event listening has to occur first, then a persistance in the student service must match the outcome of mapping an incoming message to a given student.
