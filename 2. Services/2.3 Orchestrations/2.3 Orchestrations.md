...
Let's start with a failing test to materilaize our idea here:
...
In the test above, we are verifying that any of the four aforementioned exceptions on occurance, they would be mapped into a `StudentOrchestrationDependencyValidationException`. We maintain the original localized exception as an inner exception. But we unwrap the categorical exception at this level to maintain the original issue as we go upstream.
...
Let's take a look at the possible variants for orchestration services and where they would be positioned:
 ...
 ### 2.3.4.0 Unit of Work
 With the variations of orchestration services, I highly recommend staying true to the unit of work concept. Where every request can do one thing and one thing only including it's pre-requisits. For instnace, if you need to register a student in some school, You may require also adding a guardian, contact information and some other details. Eventing these actions can greatly decrease the complexity of the flow and lower the risk of any failures in downstream services.
 ...
 The solution above is a working solution for registering a student. We needed to include guardian information, library cards, classes ... etc. These dependenices can be broken down in terms of eventing and allowing other services to pick up where the single-threaded services leaves off to continue the registration process. Something like this:
  ...
  The incoming request in the above example would be turned into events, where each one of these events would be notifying it's own orchestration services in a cul-de-sac manner as we discussed above in section 2.3.2.2. What that means is that a single thread is no longer responsible for the success of each and every dependency in the system. Instead, every event-listening broker would handle it's own process in a much, much simplified way.

  That approach does not guarantee an immediate response of sucess or failure to the requestor. It's an eventual-consistency pattern where the client would get an `Accepted` message or it's equivalant based on the communication protocol to let them know that a process has started but there's no guarantee of any results yet until it's done.

  It's also important to mention that an extra layer of resiliancy can be added to these events by temporarily storing them in Queue-like components or memory-based temporary storages depends on the criticality of the business.
